...
export async function POST(request: NextRequest) {
  try {
    ...
    const garmentType = body.garmentType || 'top'; // 'top' or 'bottom'
    ...
    if (garmentType !== 'top' && garmentType !== 'bottom') {
      return NextResponse.json(
        { error: 'Garment type must be "top" or "bottom"' },
        { status: 400 }
      );
    }
    ...
    // Construct the prompt based on garment type
    let prompt: string;
    if (garmentType === 'top') {
      prompt = 'Extract only the shirt, blouse, or top clothing item from this image. Remove the person, background, and all other elements. Show only the garment itself - preserve all details, patterns, textures, buttons, and design elements. Place it on a solid white background (#FFFFFF). The garment should appear as if it is laid flat or on a hanger, showing its full shape and design. Do not include any body parts, accessories, or background elements.';
    } else {
      prompt = 'Extract only the pants, skirt, or bottom clothing item from this image. Remove the person, background, and all other elements. Show only the garment itself - preserve all details, patterns, textures, and design elements. Place it on a solid white background (#FFFFFF). The garment should appear as if it is laid flat or on a hanger, showing its full shape and design. Do not include any body parts, accessories, or background elements.';
    }
    ...
    if (content && content.parts) {
      for (const part of content.parts) {
        ...
        // Check for inlineData (image data)
          ...
          // Use PNG mime type for output with white background
    ...
    if (!imageUrl || typeof imageUrl !== 'string') {
      ...
      return NextResponse.json(
        { error: 'Failed to extract garment - the API response did not contain an image. Please try again.' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ 
      imageUrl,
      garmentType,
      message: 'Garment extracted successfully'
    });
  } catch (error: any) {
    ...
    return NextResponse.json(
      { error: errorMessage || 'Failed to extract garment. Please try again.' },
      { status: 500 }
    );
  }
}
