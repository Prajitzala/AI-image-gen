...
export async function POST(request: NextRequest) {
  try {
    ...
    // Construct the prompt for pose normalization
    const prompt = 'Convert the person in this image to a simple standing straight pose. The person should be standing upright with arms at their sides, facing forward. Remove the background and replace it with a solid white background (#FFFFFF). Keep the person\'s proportions and appearance natural, but ensure they are in a standard standing pose.';
    ...
    if (!imageUrl || typeof imageUrl !== 'string') {
      ...
      return NextResponse.json(
        { error: 'Failed to normalize pose - the API response did not contain an image. Please try again.' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ 
      imageUrl,
      message: 'Pose normalized successfully'
    });
  } catch (error: any) {
    console.error('Error normalizing pose:', error);
    ...
    return NextResponse.json(
      { error: errorMessage || 'Failed to normalize pose. Please try again.' },
      { status: 500 }
    );
  }
}
